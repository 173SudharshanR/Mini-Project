...
app.get("/login", (req, res) => {
  let query =
    ...
    loginData.user + "' and pin = '"+loginData.pass+"';";
...
app.get("/TransferMoney", (req, res) => {
  let query1 =
    " UPDATE accounts SET balance=balance - "+transferData.amm+" where account_number='" +
    transferData.acc1+"';";
  let query2=" UPDATE accounts SET balance=balance + "+transferData.amm+" where account_number='" +transferData.acc2+"';";

  let query3="SELECT transaction_id from transactions ORDER BY transaction_id DESC limit 1;"
  ...
  con.query(query3, function (error, results) {
    ...
    console.log(n);
    let query4="INSERT INTO bankdb.transactions (transaction_id, account_number, amount, date, time, description) VALUES ('"+n+"','"+transferData.acc1+"','"+transferData.amm+"',CURDATE(),CURRENT_TIMESTAMP(),'sent to "+transferData.acc2+"');";
    con.query(query4, function (error, results) {
      if (error) throw error;
      console.log(results);
    });
    n=n+1;
    let query5 = "INSERT INTO bankdb.transactions (transaction_id, account_number, amount, date, time, description) VALUES ('"+n+"','"+transferData.acc2+"','"+transferData.amm+"',CURDATE(),CURRENT_TIMESTAMP(),'received from "+transferData.acc1+"');";
...
app.get("/insurance", (req, res) => {
  let query =
    "SELECT * FROM insurance WHERE user_id = '" +
    userID +
    "';";
